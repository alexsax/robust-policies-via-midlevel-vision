...
class Task(object):
    ...
    def restore_state(self, state: Tuple[bytes, int]) -> None:
        objs = self.get_base().get_objects_in_tree(exclude_base=False)
        if len(objs) != state[1]:
            print("Warning! mismatch between #objs started with and after. If this is NOT a procedural task, you should be concerned", len(objs), state[1])
            # raise RuntimeError(
    ...
    def get_full_state(self):
        #!!!!! DANGER!!! NOT DOING SANITY CHECK OF CHECKING FOR LEN OBJS
        arm_pos =  self.robot.arm.get_joint_positions()
        gripper_pos = self.robot.gripper.get_joint_positions()
        return self.get_base().get_configuration_tree(), self.robot.arm.get_configuration_tree(), self.robot.gripper.get_configuration_tree(), arm_pos, gripper_pos

    def restore_full_state(self, state):
        base, arm, gripper, arm_pos, gripper_pos = state
        self.robot.gripper.release()
        self.pyrep.set_configuration_tree(arm)
        self.pyrep.set_configuration_tree(gripper)
        self.robot.arm.set_joint_positions(arm_pos)
        ...
        self.pyrep.set_configuration_tree(base)
        # [self.pyrep.step_ui() for _ in range(20)]
    #####################
    # Private functions #
    #####################
