...
class TaskEnvironment(object):
    ...
    def reset(self, force_randomly_place=False, force_change_position=False) -> (List[str], Observation):
        ...
        try:
            desc = self._scene.init_episode(
                ...
                randomly_place=force_randomly_place,
                change_position=force_change_position
                )
    ...
    def _path_action_noq(self, action):
        self._assert_unit_quaternion(action[3:])

        try:
            path = self._robot.arm.get_linear_path(
                action[:3], quaternion=action[3:], ignore_collisions=True)
            done = False
            while not done:
                done = path.step()
    ...
    def step(self, action) -> (Observation, int, bool):
        ...
        if self._action_mode.arm == ArmActionMode.ABS_JOINT_VELOCITY:
            ...
            #0 new pos
            k = np.linalg.norm([a_x, a_y, a_z])
            a_x, a_y, a_z = list(np.array([a_x, a_y, a_z]) / (k*1000))
            new_pose = [x+a_x, y+a_y, z+a_z ] + [*old]
            try:
            ...
            qw, qx, qy, qz = list(new_rot)
            new_pose = [a_x + x, a_y + y, a_z + z] + [qx, qy, qz, qw]
            self._path_observations = self._path_action(new_pose)

        elif self._action_mode.arm == ArmActionMode.DELTA_EE_POSE_PLAN_NOQ:
            # self._assert_action_space(arm_action, (3,))
            a_x, a_y, a_z = arm_action * 5/100
            x, y, z, qx, qy, qz, qw = self._robot.arm.get_tip().get_pose()
            new_pose = [a_x + x, a_y + y, a_z + z] + [qx,qy,qz,qw]
            try:
                self._path_action_noq(list(new_pose))
            except:
                pass
        ...
        if current_ee != ee_action:
            ...
            while not done:
                done = self._robot.gripper.actuate(ee_action, velocity=1)
                self._pyrep.step()
                self._task.step()
            if ee_action == 0.0:
