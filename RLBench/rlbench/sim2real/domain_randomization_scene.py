...
class DomainRandomizationScene(Scene):
    ...
    def __init__(self, pyrep: PyRep, robot: Robot,
        ...
        #Temp comment out these lines, to stop from randomizing textures of gripper itself.
        if self._visual_rand_config.apply_arm:
            self._scene_objects += self._robot.arm.get_visuals()
        if self._visual_rand_config.apply_gripper:
            self._scene_objects += self._robot.gripper.get_visuals()
        if self._visual_rand_config is not None:
            if self._visual_rand_config.apply_floor:
    ...
    def _randomize(self):
        ...
        # print([k.get_name() for k in tree])
        if self._visual_rand_config is not None:
            tree = [obj for obj in tree if self._visual_rand_config.should_randomize(obj.get_name())]
