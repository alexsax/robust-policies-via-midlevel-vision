...
class TransformObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env, transform):
        ...
        exobs = env.reset()
        exobs = self.transform(exobs)
        spacedict = {}
        try:
            for key, value in exobs.items():
                if key != "save_state":
                    spacedict[key] = spaces.Box(low=-np.inf, high=np.inf,shape=value.shape)
            self.observation_space = spaces.Dict(spacedict)
